  ...
  return (
    ...
    >
      <div className={`transition-all duration-500 ${isEditing ? 'md:mr-[350px]' : ''} ${layoutMode === 'two-column' ? 'md:flex md:flex-col md:flex-grow md:min-h-0' : ''}`}>
        ...
        <div className={`sticky top-0 w-full flex justify-between items-center p-2 md:p-4 z-[100] print:hidden border-b ${theme === 'dark' ? 'bg-[#0d0d0d] border-gray-800' : 'bg-white border-gray-200 shadow-sm'} ${layoutMode === 'two-column' ? 'md:flex-shrink-0' : ''}`}>
          
          {/* Левая часть (Назад и навигация) */}
          <div className="flex gap-2 md:gap-4 items-center">
            <Link href="/setlists" className={`p-1.5 md:p-2 rounded-lg border transition-colors ${theme === 'dark' ? 'bg-black border-gray-800 text-gray-400 hover:text-blue-500' : 'bg-white border-gray-200 text-gray-600'}`}>
              <span className="text-sm md:text-base">←</span>
            </Link>
            <div className={`flex items-center rounded-lg border p-0.5 md:p-1 ${theme === 'dark' ? 'bg-black border-gray-800' : 'bg-white border-gray-200'}`}>
              <button onClick={goToPrev} disabled={currentIndex === 0} className="px-2 md:px-3 py-1 text-blue-500 disabled:opacity-20 text-xs md:text-base">←</button>
              <div className={`px-2 md:px-4 border-x font-mono text-[8px] md:text-[10px] ${theme === 'dark' ? 'border-gray-800 text-gray-500' : 'border-gray-200 text-gray-500'}`}>{currentIndex + 1} / {songs.length}</div>
              <button onClick={goToNext} disabled={currentIndex === songs.length - 1} className="px-2 md:px-3 py-1 text-blue-500 disabled:opacity-20 text-xs md:text-base">→</button>
            </div>
          </div>
          
          {/* Центр (Название сетлиста - только на десктопе) */}
          ...
          {/* Правая часть (Метроном, Настройки, Редактировать) */}
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative">
              <button 
                ...
                className={`w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full border transition-all ${showSettings ? 'bg-blue-600 rotate-90 border-blue-400 shadow-[0_0_15px_rgba(37,99,235,0.4)] text-white' : (theme === 'dark' ? 'border-gray-800 bg-[#111] text-white' : 'border-gray-200 bg-white shadow-sm text-black')}`}
              >
              ...
              {showSettings && (
                <div className={`absolute right-0 top-10 md:top-12 w-64 md:w-72 border rounded-[24px] md:rounded-[32px] p-4 md:p-6 shadow-2xl z-[150] ${theme === 'dark' ? 'bg-[#0d0d0d] border-gray-800 shadow-black' : 'bg-white border-gray-200'}`}>
                  ...
                  <div className="space-y-4 md:space-y-6">
                    <div>
                      <p className="text-[8px] md:text-[9px] text-gray-500 uppercase font-black mb-2 md:mb-3 tracking-widest text-center">ТЕМА</p>
                      <div className={`flex p-1 rounded-xl border ${theme === 'dark' ? 'bg-black border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
                        <button onClick={() => setTheme('dark')} className={`flex-1 py-1.5 md:py-2 text-[9px] md:text-[10px] font-black rounded-lg transition-all ${theme === 'dark' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400'}`}>ТЕМНА</button>
                        <button onClick={() => setTheme('light')} className={`flex-1 py-1.5 md:py-2 text-[9px] md:text-[10px] font-black rounded-lg transition-all ${theme === 'light' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400'}`}>СВІТЛА</button>
                      </div>
                    </div>
                    
                    <div className="hidden md:block">
                      <p className="text-[8px] md:text-[9px] text-gray-500 uppercase font-black mb-2 md:mb-3 tracking-widest text-center">ВИГЛЯД (DESKTOP)</p>
                      <div className={`flex p-1 rounded-xl border ${theme === 'dark' ? 'bg-black border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
                        <button onClick={() => setLayoutMode('single')} className={`flex-1 py-1.5 md:py-2 text-[9px] md:text-[10px] font-black rounded-lg transition-all ${layoutMode === 'single' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400'}`}>1 СТОВПЕЦЬ</button>
                        <button onClick={() => setLayoutMode('two-column')} className={`flex-1 py-1.5 md:py-2 text-[9px] md:text-[10px] font-black rounded-lg transition-all ${layoutMode === 'two-column' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400'}`}>2 СТОВПЦІ</button>
                      </div>
                    </div>

                    <div>
                      <p className="text-[8px] md:text-[9px] text-gray-500 uppercase font-black mb-2 md:mb-3 tracking-widest text-center">ТРАНСПＯНУВАТИ</p>
                      <div className={`flex items-center justify-between px-3 md:px-4 py-1 md:py-2 rounded-xl border ${theme === 'dark' ? 'bg-black border-gray-800' : 'bg-gray-50 border-gray-200'}`}>
                        <button onClick={() => saveSemitones(semitones - 1)} className="text-xl md:text-2xl font-bold text-blue-500 hover:scale-110 transition-transform">−</button>
                        <span className="font-bold font-mono text-lg md:text-xl text-blue-400">{semitones > 0 ? `+${semitones}` : semitones}</span>
                        <button onClick={() => saveSemitones(semitones + 1)} className="text-xl md:text-2xl font-bold text-blue-500 hover:scale-110 transition-transform">+</button>
                      </div>
                    </div>
                    <div>
                      <p className="text-[8px] md:text-[9px] text-gray-500 uppercase font-black mb-2 md:mb-3 tracking-widest text-center">КАПОДАСТР</p>
                      <div className="grid grid-cols-5 gap-1.5 md:gap-2">
                        {[0, 1, 4, 6, 9].map((val) => (
                          <button key={val} onClick={() => setCapo(val)} className={`h-8 md:h-10 rounded-lg md:rounded-xl border transition-all ${capo === val ? 'bg-blue-600 border-blue-400 text-white' : (theme === 'dark' ? 'bg-[#151515] border-gray-800 text-gray-400' : 'bg-white border-gray-200 text-gray-600')}`}>
                            <span className="text-[10px] md:text-[11px] font-black">{val === 0 ? 'Ø' : val}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                    {isOwner && userTeamId && (
                      <button onClick={() => setIsTeamShared(!isTeamShared)} className={`w-full py-2 md:py-3 rounded-xl border-2 transition-all flex items-center justify-center gap-2 md:gap-3 ${isTeamShared ? 'border-blue-600 bg-blue-600/10 text-blue-400' : (theme === 'dark' ? 'border-gray-800 bg-black text-gray-500' : 'border-gray-200 bg-white text-gray-400')}`}>
                        <div className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-full ${isTeamShared ? 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]' : 'bg-gray-700'}`}></div>
                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest">Для команди</span>
                      </button>
                    )}
                    <button onClick={() => { setShowSettings(false); setTimeout(() => window.print(), 100); }} className="w-full py-2 md:py-3 rounded-xl bg-blue-600 text-white font-black text-[8px] md:text-[9px] uppercase tracking-widest">Експорт PDF</button>
                  </div>
                </div>
              )}
            </div>
            ...
            {isOwner && (
              <button onClick={() => setIsEditing(!isEditing)} className={`px-3 py-1.5 md:px-6 md:py-2.5 rounded-full font-black text-[8px] md:text-[10px] uppercase tracking-widest transition-all whitespace-nowrap ${theme === 'dark' ? 'bg-white text-black' : 'bg-black text-white'}`}>
